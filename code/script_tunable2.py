...
for i, line in train_data.iterrows():
    ...
    print(ind-left+1, right-ind+1, len(context))
...
class Data_clean:
  ...
  def __init__(self, train, test, tokenizer: FullTokenizer, classes, inp_seq_len, max_seq_len, max_sent_len):
    ...
    self.train_att_right = np.zeros((self.train_x.shape[0], self.max_sent_len))
    self.train_att_left  = np.zeros((self.train_x.shape[0], self.max_sent_len))
    self.test_att_right  = np.zeros((self.test_x.shape[0] , self.max_sent_len))
    self.test_att_left   = np.zeros((self.test_x.shape[0] , self.max_sent_len))

    # self.train_x_right_final_context = np.zeros(())
    # self.train_x_left_final_context = 
    # self.test_x_right_final_context = 
    # self.test_x_left_final_context = 
...
train_data, test_data = map(lambda df: df.reindex(df[Data_clean.DATA_COLUMN].str.len().sort_values().index), [train_data, test_data])

data = Data_clean(train_data, test_data, tokenizer, classes, 300, 300, 200)
...
X_train = get_w2v_avg_embed(train_data, 'lemma_token', w2v_model)
X_test = get_w2v_avg_embed(test_data, 'lemma_token', w2v_model)
    
for ind, i in tqdm(enumerate(train_data.iterrows())):
    a = np.zeros((200, 100))
    count = 0
    for j in i[1]['left']:
        curr = np.where(train_data['Sentence']==j)
        if len(curr[0]):
            a[199-count] += X_train[curr[0][0]]
        count+=1
    # print(X[ind].shape, data.train_att_left[ind].shape)
    data.train_att_left[ind] += a.dot(X_train[ind])
    a = np.zeros((200, 100))
    count = 0
    for j in i[1]['right']:
        curr = np.where(train_data['Sentence']==j)
        if len(curr[0]):
            a[count] += X_train[curr[0][0]]
        count+=1
    data.train_att_right[ind] += a.dot(X_train[ind])

for ind, i in tqdm(enumerate(test_data.iterrows())):
    a = np.zeros((200, 100))
    count = 0
    for j in i[1]['left']:
        curr = np.where(test_data['Sentence']==j)
        if len(curr[0]):
            a[199-count] += X_test[curr[0][0]]
        count+=1
    # print(X[ind].shape, data.train_att_left[ind].shape)
    data.test_att_left[ind] += a.dot(X_test[ind])
    a = np.zeros((200, 100))
    count = 0
    for j in i[1]['right']:
        curr = np.where(test_data['Sentence']==j)
        if len(curr[0]):
            a[count] += X_test[curr[0][0]]
        count+=1
    data.test_att_right[ind] += a.dot(X_test[ind])

context_size = 5

train_x_right = data.train_x_right_context
train_x_left = data.train_x_left_context
test_x_right = data.test_x_right_context
test_x_left = data.test_x_left_context

data.train_x_right_context = np.zeros((train_x_right.shape[0], context_size, 300))
data.train_x_left_context = np.zeros((train_x_left.shape[0], context_size, 300))
data.test_x_right_context = np.zeros((test_x_right.shape[0], context_size, 300))
data.test_x_left_context = np.zeros((test_x_left.shape[0], context_size, 300))

k = data.train_att_right.argsort(axis=-1)
for i in range(data.train_att_right.shape[0]):
    data.train_x_right_context[i] = train_x_right[i][k[i][-5:]]

k = data.train_att_left.argsort(axis=-1)
for i in range(data.train_att_left.shape[0]):
    data.train_x_left_context[i] = train_x_left[i][k[i][-5:]]

k = data.test_att_right.argsort(axis=-1)
for i in range(data.test_att_right.shape[0]):
    data.test_x_right_context[i] = test_x_right[i][k[i][-5:]]

k = data.test_att_left.argsort(axis=-1)
for i in range(data.test_att_left.shape[0]):
    data.test_x_left_context[i] = test_x_left[i][k[i][-5:]]
